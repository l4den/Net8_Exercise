...
namespace api.Repository
{
    public class ExperienceRepository : IExperienceRepository
    {
        ...
        public async Task<bool> IsTimeIntervalFreeAsync(int personId, DateTime checkStart, DateTime checkEnd, int? excludeId = null)
        {
            var experiences = await GetAllByPersonIdAsync(personId);
            ...
            return !experiences.Any(exp => (checkStart <= exp.StartDate && exp.StartDate <= checkEnd) ||
        ...
        {
            return await _context.Experiences.AnyAsync(x => x.Id == id);
        }
    }
}